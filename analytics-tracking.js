(() => {
  ...
  let registrationResultTracked = false;
  let storedResultTracked = false;
  let suppressNextProfileResult = false;
  ...
  function track(eventName, parameters = {}) {
    if (eventName === "navigation_click" && parameters.button_name === "open_whatsapp") {
      ...
      parameters = { channel: "whatsapp", section_name: parameters.section_name || "finish" };
    }
  ...
  function gateScreen() {
    ...
    if (overlay.querySelector(".customer-denied-box")) return "access_denied";
  ...
  function inspectStatuses() {
    document.querySelectorAll(".customer-gate-status").forEach((element) => {
      ...
      if (errorType) {
        track("form_error", { form_name: "customer_registration", error_type: errorType });
      }

      if (/Erro ao salvar|Falha ao conectar|Não foi possível iniciar|Ative o provedor/i.test(text)) {
        track("site_error", {
  ...
  function navigationDetails(target) {
    const mappings = [
      ...
      [".customer-profile-logout", "close_or_logout_profile", "profile"]
    ];

    for (const [selector, buttonName, sectionName] of mappings) {
      if (target.closest(selector)) return { button_name: buttonName, section_name: sectionName };
    }
    return null;
  }
  ...
  function trackCustomizationError(button) {
    ...
    const hasMissingSelection = requiredGroups.some((group) => !document.querySelector(`input[name="${group}"]:checked`));
    if (hasMissingSelection) {
      track("form_error", { form_name: "product_customization", error_type: "missing_selection" });
    }
  }
  ...
  function trackDeliveryError(button) {
    ...
    if (!cart.length) {
      track("form_error", { form_name: "delivery_details", error_type: "empty_cart" });
      return;
    }

    const requiredFields = ["customerName", "customerPhone", "street", "number", "district"];
    const hasMissingField = requiredFields.some((id) => !document.getElementById(id)?.value.trim());
    if (hasMissingField) {
      track("form_error", { form_name: "delivery_details", error_type: "missing_required_fields" });
    }
  }
  ...
  document.addEventListener("submit", (event) => {
    ...
    if (errorType) track("form_error", { form_name: "customer_registration", error_type: errorType });
  }, true);
  ...
  window.addEventListener("copao:registration-completed", (event) => {
    if (!signUpTracked) {
      track("sign_up", { method: "registration" });
      signUpTracked = true;
    }

    if (!registrationResultTracked) {
      track("age_gate_result", {
        result: event.detail?.approved ? "approved" : "rejected",
        method: "registration"
      });
      registrationResultTracked = true;
      suppressNextProfileResult = true;
    }
  });

  window.addEventListener("copao:age-gate-stored-result", (event) => {
    if (storedResultTracked) return;
    track("age_gate_result", {
      result: event.detail?.approved ? "approved" : "rejected",
      method: "stored_profile"
    });
    storedResultTracked = true;
  });

  window.addEventListener("copao:customer-profile", () => {
    if (suppressNextProfileResult) {
      suppressNextProfileResult = false;
      return;
    }
    if (storedResultTracked) return;
    track("age_gate_result", { result: "approved", method: "stored_profile" });
    storedResultTracked = true;
  });

  window.addEventListener("error", (event) => {
    const source = String(event.filename || "inline_script").split("/").pop().split("?")[0];
    track("site_error", { script_name: source || "inline_script", error_type: "runtime_error" });
  });

  window.addEventListener("unhandledrejection", () => {
    track("site_error", { script_name: "promise", error_type: "unhandled_rejection" });
  });
